...
class FirebaseFamilyServiceImpl @Inject constructor(
    ...
    override suspend fun getChildrenDetailsList(groupName: String): List<ChildDetailsFb> {
        val usersSnapshot = firebaseDatabase.getReference(DbReference.USERS)
            .orderByChild(DbReference.User.GROUP_NAME)
            .equalTo(groupName)
            .awaitSingleValueEvent()

        val children = usersSnapshot.children.mapNotNull { it.toChildFbOrNull(groupName) }

        val detailsList = mutableListOf<ChildDetailsFb>()
        for (child in children) {
            val childId = child.uid ?: continue
